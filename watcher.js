...
const APPLICANT_NAME = env.APPLICANT_NAME || "홍길동";
const APPLICANT_PASSWORD = env.APPLICANT_PASSWORD || "1234";
const APPLICANT_SEX = (env.APPLICANT_SEX || "M").toUpperCase();
const BIRTH_YYYYMMDD = env.BIRTH_YYYYMMDD || "19900101";
...
if (
  ...
  !APPLICANT_SEX ||
  !BIRTH_YYYYMMDD ||
  BIRTH_YYYYMMDD.length !== 8
) {
  logStep(
    "env_check_failed",
    "APPLICANT_NAME/APPLICANT_PASSWORD/APPLICANT_SEX/BIRTH_YYYYMMDD missing",
  );
  console.log(
    JSON.stringify({
      ok: false,
...
const SELECTORS = {
  ...
  submit:
    '#btn_p_form, a[onclick*="formcheck"], button:has-text("신청"), input[type="submit"]',
  general: "text=일반",
  full: "#Full",
};
...
(async () => {
  ...
  try {
    ...
    const yearOptions = await page
      .locator('select[name="birth_year"] option')
      .allTextContents()
      .catch(() => []);
    const monthOptions = await page
      .locator('select[name="birth_month"] option')
      .evaluateAll((opts) => opts.map((o) => (o.value || "").trim()))
      .catch(() => []);
    const dayOptions = await page
      .locator('select[name="birth_day"] option')
      .evaluateAll((opts) => opts.map((o) => (o.value || "").trim()))
      .catch(() => []);
    logStep(
      "birth_options_probe",
      `year_opts=${yearOptions.length}, month_vals=${monthOptions.slice(0, 5).join(",")}, day_vals=${dayOptions.slice(0, 5).join(",")}`,
    );

    await page
      .selectOption('select[name="birth_year"]', { value: by })
      .catch(() => {});
    const monthVal = monthOptions.includes(bm2)
      ? bm2
      : String(parseInt(bm2, 10));
    const dayVal = dayOptions.includes(bd2) ? bd2 : String(parseInt(bd2, 10));
    await page
      .selectOption('select[name="birth_month"]', { value: monthVal })
      .catch(() => {});
    await page
      .selectOption('select[name="birth_day"]', { value: dayVal })
      .catch(() => {});

    if (APPLICANT_SEX === "F") {
      await page
        .locator(SELECTORS.female)
        .first()
        .click({ force: true })
        .catch(() => {});
    } else {
      await page
        .locator(SELECTORS.male)
        .first()
        .click({ force: true })
        .catch(() => {});
    }
    ...
    logStep(
      "form_filled",
      `name=${APPLICANT_NAME}, sex=${APPLICANT_SEX}, birth=${by}-${monthVal}-${dayVal}, sexState=${JSON.stringify(sexState)}`,
    );
    ...
    for (const p of candidatePages) {
      ...
      for (const f of frameList) {
        const gCount = await f
          .locator(SELECTORS.general)
          .count()
          .catch(() => 0);
        const rCount = await f
          .locator('input[type="radio"]')
          .count()
          .catch(() => 0);
        logStep(
          "scan_frame",
          `url=${f.url()} general=${gCount} radios=${rCount}`,
        );
    ...
    const dumpInputs = await workPage
      .evaluate(() => {
    ...
    logStep("full_title_probe_start");
    const fullTitle = await workPage.evaluate(() => {
      const el = document.querySelector("input#Full");
